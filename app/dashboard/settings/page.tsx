...
const AccountSettings = () => {
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUser((prevUser) => prevUser ? { ...prevUser, [name]: value } as IUser : null);
  };
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} sm={4}>
              <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                <Avatar sx={{ width: 80, height: 80, mb: 2 }} src={user?.image} alt="Profile" />
            ...
            <Grid item xs={12} sm={8}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <TextField fullWidth label="Middle Name" name="middleName" value={user?.middleName || ""} onChange={handleInputChange} />
                </Grid>
                <Grid item xs={12}>
                  <TextField fullWidth label="Last Name" name="lastName" value={user?.lastName || ""} onChange={handleInputChange} />
                </Grid>
                <Grid item xs={6}>
                  <TextField fullWidth label="Username" name="username" value={user?.username || ""} onChange={handleInputChange} />
                </Grid>
                <Grid item xs={6}>
                  <TextField fullWidth label="Phone Number" name="mobileNumber" value={user?.mobileNumber || ""} onChange={handleInputChange} />
                </Grid>
                <Grid item xs={12}>
                  <TextField fullWidth label="Email Address" name="email" value={user?.email || ""} disabled />
                </Grid>
                <Grid item xs={6}>
                  <TextField fullWidth label="NIN Number" name="nin" value={user?.nin || ""} onChange={handleInputChange} />
                </Grid>
                <Grid item xs={6}>
                  <TextField fullWidth label="BVN" name="bvn" value={user?.bvn || ""} onChange={handleInputChange} />
                </Grid>
              </Grid>
            </Grid>
          </Grid>
